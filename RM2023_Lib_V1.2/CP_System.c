...
uint8_t CP_send_buf[256];              // 给裁判系统发送数组
volatile uint32_t sysTickUptime;
volatile uint32_t usTicks = 0;
uint32_t DUM_Connect_time = 0; // 上下主控联系
uint8_t CP_Online = 0;         // 裁判系统在线标志
uint8_t CP_watchdog = 0;
uint16_t CP_loss_100ms = 0;

float now_heat = 0; // 本地热量数据，裁判系统数据更新后会修正此值
float CP_RXlast_heat = 0;
uint32_t heat_cnt = 0;
CP_typedef CP;
void CP_watchdog_run(void)
{
    CP_loss_100ms++;
    if (CP_loss_100ms > 5)
    {
...
void CP_System_IRQHandler(DMA_HandleTypeDef *hdma)
{
    ...
    if ((tmp_flag != RESET))
    {
        ...
        //     CP_FIFO = 1;
        CP_FIFO = !CP_FIFO;
        HAL_UART_Receive_DMA(&CP_SYSTEM_USART_HANDLE, (uint8_t *)CP_rev_buf[CP_FIFO], CP_RX_BUF_SIZE);
        CP_data_deal();
    }
}

void CP_System_DMACplt_DataDeal(void)
{
    uint16_t ulen = 0;
...
void CP_Delete_Graphic(uint8_t operate_tpye, uint8_t layer) // operate_tpye参数：0为空操作，1为删除某个图层，2为删除所有
{
    ...
    *(uint16_t *)&CP_send_buf[11] = CP.ext_student_interactive_header_data_t.receiver_ID;
    ...
    HAL_UART_Transmit_DMA(&CP_SYSTEM_USART_HANDLE, CP_send_buf, 17);
}
                                 ...
                                 uint16_t radius, uint16_t end_x, uint16_t end_y)
{
    ...
    CP.cmd_id = 0x0301;
    CP.ext_student_interactive_header_data_t.data_cmd_id = 0x0101;                                  // 字内容id
    CP.ext_student_interactive_header_data_t.send_ID = Send_ID;                                     // 发送方id
    CP.ext_student_interactive_header_data_t.receiver_ID = Receiver_ID;                             // 接收方id
    CP.ext_client_custom_graphic_single_t.grapic_data_struct.graphic_name[0] = graphic_name & 0xff; // 图形名
    ...
    CP.ext_client_custom_graphic_single_t.grapic_data_struct.operate_tpye = operate_tpye; // ui图形操作
    CP.ext_client_custom_graphic_single_t.grapic_data_struct.graphic_tpye = graphic_tpye; // ui图形类型
    CP.ext_client_custom_graphic_single_t.grapic_data_struct.layer = layer;               // 图层 bit 6-9,max=9
    ...
    *(uint32_t *)&CP_send_buf[24] =
    ...
    HAL_UART_Transmit_DMA(&CP_SYSTEM_USART_HANDLE, CP_send_buf, 30);
}
                                 ...
                                 uint16_t radius[2], uint16_t end_x[2], uint16_t end_y[2])
{
    ...
    *(uint16_t *)&CP_send_buf[11] = CP.ext_student_interactive_header_data_t.receiver_ID = Receiver_ID;
    ...
    HAL_UART_Transmit_DMA(&CP_SYSTEM_USART_HANDLE, CP_send_buf, 45);
}
                                  ...
                                  uint16_t radius[5], uint16_t end_x[5], uint16_t end_y[5])
{
    ...
    *(uint16_t *)&CP_send_buf[11] = CP.ext_student_interactive_header_data_t.receiver_ID = Receiver_ID;
    ...
    HAL_UART_Transmit_DMA(&CP_SYSTEM_USART_HANDLE, CP_send_buf, 90);
}
                                   ...
                                   uint16_t radius[7], uint16_t end_x[7], uint16_t end_y[7])
{
    ...
    *(uint16_t *)&CP_send_buf[11] = CP.ext_student_interactive_header_data_t.receiver_ID = Receiver_ID;
    ...
    HAL_UART_Transmit_DMA(&CP_SYSTEM_USART_HANDLE, CP_send_buf, 120);
}
...
void CP_DrawOrDelete_One_Number(uint32_t graphic_name, operate_tpyedef operate_tpye, graphic_tpyedef graphic_tpye, uint8_t layer, Color_tpyedef color,
                                uint16_t number_size, uint16_t number_digit, uint16_t width, uint16_t start_x, uint16_t start_y, // number_digit为小数位数
                                float number)
{
    CP.frame_header.SOF = 0xA5;
    CP.frame_header.data_length = 6 + 15;
    ...
    *(uint32_t *)&CP_send_buf[20] =
    ...
    if (graphic_tpye == Graphic_Int_number)
        *(int32_t *)&CP_send_buf[24] = number;
    else
        *(int32_t *)&CP_send_buf[24] = number * 1000;
                          ...
                          uint8_t *data) //''不行，""行
{
    ...
    *(uint32_t *)&CP_send_buf[24] = 0;
    ...
    HAL_UART_Transmit_DMA(&CP_SYSTEM_USART_HANDLE, CP_send_buf, 60);
}
        ...
        if (CP_rev_buf_point[0 + CP_rev_buf_tempindex] == 0xA5 && CP_rev_buf_point[4 + CP_rev_buf_tempindex] == Get_CRC8_Check_Sum(&CP_rev_buf_point[0 + CP_rev_buf_tempindex], 4))
        {
            ...
            if (*(uint16_t *)(CP_rev_buf_point + 5 + 2 + CP.frame_header.data_length + CP_rev_buf_tempindex) == Get_CRC16_Check_Sum(&CP_rev_buf_point[0 + CP_rev_buf_tempindex], 5 + 2 + CP.frame_header.data_length))
            {
                ...
                case 0x0001:
                    ...
                    CP.ext_game_status_t.game_progress = CP_rev_buf_point[0 + 5 + 2 + CP_rev_buf_tempindex] >> 4; // 当前比赛阶段0未开始，1准备，2自检15s，3倒计时5s，4比赛中，5比赛结算
                ...
                case 0x0201:
                    ...
                    CP.ext_game_robot_status_t.current = CP_rev_buf_point[3 + 5 + 2 + CP_rev_buf_tempindex] << 8 | CP_rev_buf_point[2 + 5 + 2 + CP_rev_buf_tempindex];
                    CP.ext_game_robot_status_t.maximum_HP = CP_rev_buf_point[5 + 5 + 2 + CP_rev_buf_tempindex] << 8 | CP_rev_buf_point[4 + 5 + 2 + CP_rev_buf_tempindex];
                    CP.ext_game_robot_status_t.shooter_id1_17mm_barrel_cooling_value = CP_rev_buf_point[7 + 5 + 2 + CP_rev_buf_tempindex] << 8 | CP_rev_buf_point[6 + 5 + 2 + CP_rev_buf_tempindex];
                    CP.ext_game_robot_status_t.shooter_id1_17mm_barrel_heat_limit = CP_rev_buf_point[9 + 5 + 2 + CP_rev_buf_tempindex] << 8 | CP_rev_buf_point[8 + 5 + 2 + CP_rev_buf_tempindex];
                    CP.ext_game_robot_status_t.chassis_power_limit = CP_rev_buf_point[11 + 5 + 2 + CP_rev_buf_tempindex] << 8 | CP_rev_buf_point[10 + 5 + 2 + CP_rev_buf_tempindex];
                    CP.ext_game_robot_status_t.mains_power_gimbal_output = CP_rev_buf_point[12 + 5 + 2 + CP_rev_buf_tempindex] & 0x01;
                    CP.ext_game_robot_status_t.mains_power_chassis_output = (CP_rev_buf_point[12 + 5 + 2 + CP_rev_buf_tempindex] & 0x02) >> 1;
                    CP.ext_game_robot_status_t.mains_power_shooter_output = (CP_rev_buf_point[12 + 5 + 2 + CP_rev_buf_tempindex] & 0x04) >> 2;
                ...
                case 0x0202:
                    ...
                    /*裁判系统数据，更新10hz，但是有效变化仅仅5HZ*/
                    if (CP_RXlast_heat != CP.ext_power_heat_data_t.shooter_id1_17mm_cooling_heat)
                    {
                        now_heat = CP.ext_power_heat_data_t.shooter_id1_17mm_cooling_heat;
                    }
                    CP_RXlast_heat = CP.ext_power_heat_data_t.shooter_id1_17mm_cooling_heat;
                    // heat_cnt++;
                ...
                case 0x0203:
                    ...
                    memcpy((void *)&CP.ext_game_robot_pos_t.angle, (void *)&CP_rev_buf_point[8 + 5 + 2 + CP_rev_buf_tempindex], sizeof(CP.ext_game_robot_pos_t.angle));
                    /***********************************************************************************************************************************/
                ...
                case 0x0209:
                    CP.ext_rfid_status_t.rfid_status = CP_rev_buf_point[4 + 5 + 2 + CP_rev_buf_tempindex] << 31 | CP_rev_buf_point[3 + 5 + 2 + CP_rev_buf_tempindex] << 24 | CP_rev_buf_point[2 + 5 + 2 + CP_rev_buf_tempindex] << 16 | CP_rev_buf_point[1 + 5 + 2 + CP_rev_buf_tempindex] << 8 | CP_rev_buf_point[0 + 5 + 2 + CP_rev_buf_tempindex];
                    /***********************************************************************************************************************************/
                ...
                case 0x0301:
                    ...
                    uint16_t temp_cnt;
                    for (temp_cnt = 0; temp_cnt < CP.frame_header.data_length - 6; temp_cnt++)
                        CP.robot_interactive_data_t.data[CP.frame_header.data_length - 6 - 1 - temp_cnt] = CP_rev_buf_point[temp_cnt + 6 + 5 + 2 + CP_rev_buf_tempindex];
                ...
                case 0x0303:
                    ...
                    CP.ext_robot_command_t.cmd_keyboard = CP_rev_buf_point[8 + 5 + 2 + CP_rev_buf_tempindex];
                    CP.ext_robot_command_t.target_robot_id = CP_rev_buf_point[9 + 5 + 2 + CP_rev_buf_tempindex];
                    CP.ext_robot_command_t.cmd_source = CP_rev_buf_point[11 + 5 + 2 + CP_rev_buf_tempindex] << 8 | CP_rev_buf_point[10 + 5 + 2 + CP_rev_buf_tempindex];
                    /***********************************************************************************************************************************/
                    break; // 小地图下发信息，触发时发送
                case 0x0304:
