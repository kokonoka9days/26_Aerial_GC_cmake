...
#define CP_SYSTEM_USART_HANDLE huart1
...
#define Receiver_ID (Server_ID_tpyedef)(CP.ext_game_robot_status_t.robot_id + 256)
    ...
    // 修订命令码0x0003、0x0101、0x0105、0x0201、0x0202、0x0204、0x0209、0x020B、0x020C
    // 删除命令码0x0102、0x0205
    // 新增命令码0x0309
    /**底盘裁判系统通信，延迟130ms，丢包率小于1% */
    typedef struct CP_typeStruct
    {
        // 通信协议格式 frame_header,cmd_id,data,frame_tail(2byte,crc16,整包校验)
        ...
        /* 命令码id */
        uint16_t cmd_id;
        struct /**ID 0x0001 机器人血量数据*/
        {
            uint8_t game_type : 4;
            uint8_t game_progress : 4;
            uint16_t stage_remain_time;
        } ext_game_status_t;
        struct /**ID 0x0002 比赛结果数据*/
        {
            uint8_t winner;
        } ext_game_result_t;
        struct /**ID 0x0003 比赛结果数据*/
        {
            // 未上场以及罚下血量为 0
        ...
        {
            uint32_t event_type;
        } ext_event_data_t; // 场地事件数据（己方停机坪占领状态，己方能量机关状态，己方能量机关状态）
        struct
        {
        ...
        struct
        {
            ...
            uint16_t current;    // 机器人剩余血量
            uint16_t maximum_HP; // 机器人上限血量

            uint16_t shooter_id1_17mm_barrel_cooling_value; // 机器人 17mm 1 号 枪口每秒冷却值
            uint16_t shooter_id1_17mm_barrel_heat_limit;    // 机器人  17mm 1 号 枪口热量上限

            uint16_t shooter_id2_17mm_barrel_cooling_value; // 机器人  17mm 2 号枪口每秒冷却值
            uint16_t shooter_id2_17mm_barrel_heat_limit;    // 机器人  17mm 2 号 枪口热量上限

            uint16_t shooter_id1_42mm_barrel_cooling_value; // 机器人 42mm 枪口每秒冷却值
            uint16_t shooter_id1_42mm_barrel_heat_limit;    // 机器人 42mm 枪口上限速度 单位 m/s
        ...
        struct graphic_data_struct_t
        {
            ...
            uint32_t graphic_tpye : 3; // ui图形类型
            uint32_t layer : 4;        // ui图形层级 max0-9
            uint32_t color : 4;        // ui图形颜色
            uint32_t start_angle : 9;  // ui图形起始角度
            uint32_t end_angle : 9;    // ui图形结束角度
        ...
        struct
        {
            struct graphic_data_struct_t grapic_data_struct[2];
        } ext_client_custom_graphic_double_t; // 客户端绘制二个图形
        struct
        {
            struct graphic_data_struct_t grapic_data_struct[5];
        } ext_client_custom_graphic_five_t; // 客户端绘制五个图形
        struct
        {
            struct graphic_data_struct_t grapic_data_struct[7];
...
#define CP_KEY_PRESSED_B ((uint16_t)(CP_jianpan & 0x8000) >> 15)
    ...
    extern void CP_System_Init(void);
    extern void CP_watchdog_run(void);

    extern void CP_System_IRQHandler(DMA_HandleTypeDef *hdma);
    extern void CP_System_DMACplt_DataDeal(void);
